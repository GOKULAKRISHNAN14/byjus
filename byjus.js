...
(function () {
	...
	var HOME = {
		...
		onPageLoadFunctions: function () {
			...
			// for dropdown
			document.querySelectorAll('.dropdown-menu ul>li>ul').forEach(function (item, index) {
				item.parentNode.className += 'sub';
			});
			var isMobileTablet = navigator.isMobileTablet;
			if(isMobileTablet){
				document.querySelectorAll(".big-menu").forEach( function(item, index){
						item.addEventListener('click', function (event) {
						var ele = this;
						ele.classList.toggle("menu-open");
						})
					})
					document.querySelectorAll(".sub").forEach( function(item, index){
						item.addEventListener('click', function (event) {
							var ele = this;
							ele.classList.toggle("menu-open-sub");
							event.stopPropagation();
						})
					})
			}

			// for header links reorder
			if (isMobile) {
				document.querySelector('#top-navbar-ncert-solutions-links a').setAttribute('href', '/ncert-solutions/');
				document.querySelector('#top-navbar-ncert-solutions-links .dropdown-menu').append(document.querySelector('#top-navbar-collapse .navbar-right [href="/ncert-solutions/"]').nextElementSibling);
				document.querySelector('#top-navbar-collapse .navbar-right [href="/ncert-solutions/"]').parentNode.remove()
				document.querySelector('#top-navbar-state-board-links a').setAttribute('href', '/education-boards-in-india/');
				document.querySelector('#top-navbar-state-board-links .dropdown-menu').append(document.querySelector('#top-navbar-collapse .navbar-right [href="/education-boards-in-india/"]').nextElementSibling);
				document.querySelector('#top-navbar-collapse .navbar-right [href="/education-boards-in-india/"]').parentNode.remove()
				document.querySelector('#top-navbar-jee-links a').setAttribute('href', '/jee/');
				document.querySelector('#top-navbar-jee-links .dropdown-menu').append(document.querySelector('#top-navbar-collapse .navbar-right [href="/jee/"]').nextElementSibling);
				document.querySelector('#top-navbar-collapse .navbar-right [href="/jee/"]').parentNode.remove()
			}
			

			// for toggling class
			// data-toggle="class" data-class-name="animate" data-target="#top-navbar-collapse"
			document.querySelectorAll('[data-toggle="class"]').forEach(function (item, index) {
				item.addEventListener('click', function (event) {
					event.preventDefault();
					var ele = this;
					var eleClassname = this.getAttribute('data-class-name');
					var toEle = document.querySelector(this.getAttribute('data-class-target'));

					if (toEle.className) {
						var classs = toEle.className.split(' ');
						if (classs.indexOf(eleClassname) != -1) {
							var openI = classs.indexOf(eleClassname);
							classs.splice(openI, 1);
						} else {
							classs.push(eleClassname);
						}
						toEle.className = classs.join(' ');
					}
				});
			});
			...
			// ga
			if (GA_FLAG) {
				document.querySelectorAll('#top-navbar a') && document.querySelectorAll('#top-navbar a').forEach(function (item, index) {
